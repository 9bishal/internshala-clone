...
async function insertTestData() {
  try {
    ...
    for (const job of testJobs) {
      ...
      jobCount++;
      console.log(`   ✅ Job ${jobCount}: "${job.title}" at ${job.company} (ID: ${docRef.id})`);
    }
    console.log(`\n✅ Successfully inserted ${jobCount} test jobs\n`);

    console.log('📝 Inserting test internships...');
    let internshipCount = 0;
    for (const internship of testInternships) {
      const docRef = await db.collection('internships').add(internship);
      internshipCount++;
      console.log(`   ✅ Internship ${internshipCount}: "${internship.title}" at ${internship.company} (ID: ${docRef.id})`);
    }
    console.log(`\n✅ Successfully inserted ${internshipCount} test internships\n`);

    console.log('🎉 All test data inserted successfully!');
    console.log('📊 Summary:');
    console.log(`   - Jobs inserted: ${jobCount}`);
    console.log(`   - Internships inserted: ${internshipCount}`);
    console.log(`   - Total records: ${jobCount + internshipCount}`);
...
// Start the insertion with a timeout
const insertPromise = insertTestData();

// Set a timeout to force exit after 30 seconds
setTimeout(() => {
  console.error('\n⏱️  Timeout: Data insertion took too long. Force exiting...');
  process.exit(1);
}, 30000);
